...
- The extension now ships with Dark+-friendly default colors so `.collie` files look good out of the box: `collieTag` `#569CD6`, `collieClassShorthand` `#4EC9B0`, `collieDirective` `#C586C0`, `colliePropsKeyword` `#D7BA7D`, `colliePropsField` `#9CDCFE`, `collieInterpolation` `#CE9178`, `colliePipeText` `#D4D4D4`, `collieComment` `#6A9955`. Override any of them via standard `editor.semanticTokenColorCustomizations` rules if you prefer another palette.
